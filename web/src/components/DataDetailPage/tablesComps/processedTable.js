                    ...
                    <TableCell align="right">{// 1회차 
                        ...
                        ?( // minute값을 업로드하는 경우 (수정 불가 , 추가할 때 입력만 가능) -> 이미지 업로드 부분만 보면 될 것 같은데 수정 api 다 잘됨 
                            ...
                            ?modal
                                ...
                                :<input key={'processed-'+idx+'input'} style={{width:'100px',height:'23px'}} name={f} value={processedMinute[0]} 
                                    placeholder={processedMinute[0]===null?"0.0":processedMinute[0]} onChange={(e)=>{handleMinuteInputChange(e, 0);}}/>
                        ...
                        (edited
                            ?modal
                                ?<InputTransitionsModal setModal={setModal}/>
                                :<input key={'processed-'+idx+'input'} style={{width:'100px',height:'23px'}} name={f} value={processedInput[0]?.[f]} 
                                 placeholder={processed_data[0]===null?"0.0":processed_data[0]?.[f]} onChange={(e)=>{handleInputChange(e,1,0);}}/>
                            :(processedInput[0]?.[f] ? processedInput[0]?.[f] : "")
                        )
                    }</TableCell>
                    {
                        ...
                        Array.from({ length: Number(processedToggleValue.slice(0, -1))-1 }, (_, arr_idx) => (
                            <TableCell align="right">
                            {
                                ...
                                ?(
                                    ...
                                    ?modal
                                        ?<InputTransitionsModal setModal={setModal}/>
                                        :<input key={'processed-'+idx+'input'} name={f} style={{width:'100px',height:'23px'}} 
                                          value={processedMinute[arr_idx+1]} placeholder={processedMinute[arr_idx+1]===null?"0.0":processedMinute[arr_idx+1]} 
                                ...
                                ?modal
                                    ?<InputTransitionsModal setModal={setModal}/>
                                    :<input key={'processed-'+arr_idx+'-input'}  style={{width:'100px',height:'23px'}} name={f} 
                                        value={processedInput[arr_idx+1]?.[f]} placeholder={processed_data[arr_idx+1]===null?"0.0":processed_data[arr_idx]?.[f]} 
