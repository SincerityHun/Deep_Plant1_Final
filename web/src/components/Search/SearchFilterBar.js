...
function SearchFilterBar({setStartDate, setEndDate, setIsDateFilterClicked}){
    ...
    return(
        <div>
            ...
            >
                <Card style={styles.cardStyle}>
                        ...
                        <LocalizationProvider dateAdapter={AdapterDayjs}>
                            <DemoContainer components={["DatePicker"]} sx={{  mx: "auto" }}>
                                <div style={{display:'flex', marginLeft:'20px'}}>
                                    <DatePicker 
                                    disableFuture 
                                    onChange={(newVal)=>{setCalenderStart(newVal); setIsDur(false);}} 
                                    ...
                                    <DatePicker 
                                    disableFuture 
                                    minDate={calenderStart? calenderStart:dayjs('1970-01-01')}  
                                    onChange={(newVal)=>{setCalenderEnd(newVal); setIsDur(false);}} 
                    ...
                    <Box style={{display:'flex', justifyContent:'end'}}>
                        <button 
                            onClick={()=>{setIsDur(false); handleCompleteBtn(); setAnchorEl(null); }} 
            ...
            <IconButton
            style={{border:`1px solid ${navy}`, borderRadius:'10px', width:'35px', height:'35px', marginLeft:'10px', padding:'0px'}}
            size="small"
            onClick={()=>{setDuration('week'); setCalenderStart(null); setCalenderEnd(null); setIsDur(true); handleCompleteBtn();}}>
                <FaArrowRotateLeft/>
            </IconButton>
