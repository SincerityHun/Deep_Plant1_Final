...
class FreshMeatEvalViewModel with ChangeNotifier {
  ...
  void _initialize() {
    ...
    // 완료 체크
    if (marbling != 0 && color != 0 && texture != 0 && surface != 0 && overall != 0) {
      completed = true;
    }
    notifyListeners();
  }
    ...
    try {
      if (meatModel.seqno == 0) {
        ...
        if (meatModel.id != null) {
          // 수정
          await RemoteDataSource.sendMeatData('sensory_eval', meatModel.toJsonFresh());
        }
      } else {
        // 처리육
        meatModel.deepAgedFreshmeat ??= {};
        meatModel.deepAgedFreshmeat!['createdAt'] = Usefuls.getCurrentDate();
        meatModel.deepAgedFreshmeat!['period'] = Usefuls.getMeatPeriod(meatModel);
