...
class AddDeepAgingDataViewModel with ChangeNotifier {
  ...
  Future<void> saveData(BuildContext context) async {
    ...
    print(jsonData);
    try {
      dynamic response = await RemoteDataSource.sendMeatData('deep_aging_data', jsonData);
      if (response == null) {
        throw Error();
      } else {
        meatModel.deepAgingData!
            .add({"deepAgingNum": '$seqno회', "date": '$selectedYear$selectedMonth$selectedDay', "minute": int.parse(selectedMinute!), "complete": false});
        _context = context;
        _movePage();
      }
    } catch (e) {
