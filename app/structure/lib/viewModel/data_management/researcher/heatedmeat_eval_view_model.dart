...
class HeatedMeatEvalViewModel with ChangeNotifier {
  ...
  void _checkAllInserted() {
    if (flavor != 0 && juiciness != 0 && tenderness != 0 && umami != 0 && palatability != 0) {
      completed = true;
    } else {
      completed = false;
    }
  ...
  Future<void> saveData(BuildContext context) async {
    ...
    try {
      dynamic response = await RemoteDataSource.sendMeatData('heatedmeat_eval', meatModel.toJsonHeated());
      if (response == null) {
        throw Error();
      } else {
        _context = context;
