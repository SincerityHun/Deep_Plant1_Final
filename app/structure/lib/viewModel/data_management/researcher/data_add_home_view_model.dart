...
class DataAddHomeViewModel with ChangeNotifier {
  ...
  Future<void> deleteList(int idx) async {
    ...
    notifyListeners();
    try {
      dynamic response = await RemoteDataSource.deleteDeepAging(meatModel.id!, idx);
      if (response == null) {
        throw Error();
      } else {
  ...
  // 처리육 총 결산
  void _setTotal() {
    int totalMinutes = meatModel.deepAgingData!.map((item) => item['minute'] as int).fold(0, (sum, minute) => sum + minute);
